...
var showEditQuotaPp = function(rowData){
	...
	$('#quota_volume_end').css({'border-color': '#ebedf2'})
	...
	if(!rowData) {
		...
		// Установка дат
		$('#quota_volume_start').datepicker('setDate', new Date(topDateChanger.interval.selected_start_dtime).startOfMonth())
		$('#quota_volume_end').datepicker('setDate', null)
	}else{
		...
		// Установка дат
		$('#quota_volume_start').datepicker('setDate', rowData.start.replace(/(\d+)[-](\d+)[-](\d+)/, '$2 $1'));
		if (rowData.end != 'null') $('#quota_volume_end').datepicker('setDate', rowData.end.replace(/(\d+)[-](\d+)[-](\d+)/, '$2 $1'))
			else $('#quota_volume_end').datepicker('setDate', null)
	}
	blockModalForm()
...
var applyQuotaFromEdit = function(){
	...
	var endDtime = $('#quota_volume_end').data('datepicker').getDate() ? $('#quota_volume_end').data('datepicker').getDate().getTime() : null
	var startDtime = $('#quota_volume_start').data('datepicker').getDate() ? $('#quota_volume_start').data('datepicker').getDate().getTime() : null
	...
	if(EDIT_QUOTA_MODE != 'creation') data.quota_volume_id = QUOTA_ID
	OutRequests['set-quota-volume']({
		data: data,
		success: function(resp){
	...
	if(action == 'delete'){
		...
		OutRequests['delete-quota-volume']({
			...
			success: function(resp){
				...
				if(resp.result){
					ae$('.controls-tip').textContent = 'Вы уверены, что хотите продолжить?'
				} else {
					ae$('.controls-tip').textContent = 'Вы уверены, что хотите продолжить?'
				}
			}, 
		...
		if(EDIT_QUOTA_MODE != 'creation'){
			...
			var data = {
				...
				kind: 'max_by_square',
				value: $('#value_input').val(),
				start: $('#quota_volume_start').data('datepicker').getDate() ? $('#quota_volume_start').data('datepicker').getFormattedDate('yyyy-mm-dd') : '',
				end: $('#quota_volume_end').data('datepicker').getDate() ? $('#quota_volume_end').data('datepicker').getFormattedDate('yyyy-mm-dd') : '',
				mode: 'check',
				csrfmiddlewaretoken: csrf_token,
			}
			if(EDIT_QUOTA_MODE != 'creation') data.quota_volume_id = QUOTA_ID
			OutRequests['set-quota-volume']({
				...
				success: function(resp){
					...
					if(resp.result){
						toggleModalControls('confirm')
						ae$('.controls-tip').textContent = 'Вы изменяете значение ограничения, продолжить?'
