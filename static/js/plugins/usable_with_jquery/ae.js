...
ae$ = document.querySelector.bind(document)
ae$$ = document.querySelectorAll.bind(document)
Element.prototype.ae$ = Element.prototype.querySelector
Element.prototype.ae$$ = Element.prototype.querySelectorAll
...
Object.defineProperty(Array.prototype, 'uniqByKey', {
  value: function(key) {
...
Object.defineProperty(Array.prototype, 'aeRemove', { 
	value: function(elem) {
...
function calcPosForDropDown(targetRect, ddRect, margin){
	//params: ClientRect
	...
	if(targetRect.bottom + ddRect.height < tHeight){
		//если он влезает по ширине
		if(targetRect.left + ddRect.width < tWidth){
			x = targetRect.left
			y = targetRect.bottom
		} else {
			x = targetRect.right - ddRect.width
			y = targetRect.bottom
		}
	} else {
		if(targetRect.left + ddRect.width < tWidth){
			x = targetRect.left
			y = targetRect.top - ddRect.height - margin
		} else {
			x = targetRect.right - ddRect.width
			y = targetRect.top - ddRect.height - margin
		}
...
function showPopupUnder(elem, fillFunc){
	var wrap = document.createElement('div')
	wrap.classList.add('small-dropdown')
	fillFunc(wrap, close)
	document.body.appendChild(wrap)

	var elemRect = elem.getBoundingClientRect()
	var wrapRect = wrap.getBoundingClientRect()
	var pos = calcPosForDropDown(elemRect, wrapRect, 8)
	wrap.style.left = pos.left +'px'
	wrap.style.top = pos.top +'px'

	function close(){
		wrap.throwEvent('close')
		wrap.remove()
		document.body.removeEventListener('mousedown', onClick)
		document.body.removeEventListener('touchstart', onClick)
	}
	function onClick(e){
		if (e.target.findElem(wrap)) return //клик по попапу, скрывать не нужно
		close()
	}
...
// очистка выделенного на странице
function clearUserSelectionOnPage(){
