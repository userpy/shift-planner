...
function InitCustomSelect(el, paramsArray, superParams){
	...
	if(~el.paramsArray.indexOf('searchable')){
		...
		label.$('input').addEventListener('keydown', onSearchInputKeydown)
		label.$('input').addEventListener('keyup', onSearchInputKeyUp)
	}
	...
	el.hideOption = function(obj){
		...
		var line = dd.$('.csb-option[data-value="'+value+'"]')
	...
	el.addOption = function(obj, silent){
		...
		var dd = $('.csb-dd')
		if( !dd || dd.parentSelector != el ) dd = el
		var optionsWrap = dd.$('.csb-options-wrap')
	...
	el.selectOption = function(obj, silent){
		...
		var line
		var value = encodeURI( obj.value )
		line = dd.$('.csb-option[data-value="'+ value +'"]')
		...
		if(!silent) {
			...
			if(!~el.paramsArray.indexOf('multiSelect')){
				el.$$('.csb-option').classList.remove('selected')
			}
	...
	function doSearchOnKeyUp(input){
		...
		if(input.value == ''){
			restoreAllLevels()
			dd.$$('.csb-option').classList.remove('hidden')
			return
		}
		removeAllLevels() 
	...
	function updateSelectLabel(){
		...
		function updateInSearchable(){
			var label = el.$('.csb-label')
			var optionsWrap = label.$('.select-options')
			var searchInput = label.$('.select-search')
			searchInput.value = ''
	...
	function onChange(e){
		var dd = $('.csb-dd')
		if( !dd || dd.parentSelector != el ) dd = el
		dd.$$('[data-name]').classList.remove('selected')
		el.selectedOptions.forEach(function(opt){
			var line
	...
	function expand(){
		...
		var elOptionsWrap = el.$('.csb-options-wrap')
		var dd = createEl('div')
		...
		dd.$$('.csb-option').classList.remove('hidden')//скрыты бывают после поиска
		...
		if( ~el.paramsArray.indexOf('width-auto') ){
			...
			var width = 0
			elOptionsWrap.$$('.csb-option').forEach(function(op){
		...
		if(selectedOption && !~el.paramsArray.indexOf('multiSelect')){
			...
			var line = dd.$('.csb-option[data-name="'+ name +'"]')
			line.scrollIntoView()
		}
		if(~el.paramsArray.indexOf('searchable')){
			el.$('.select-search').focus()
		}
	}
	function close(){
		var dd = $('.csb-dd')
		...
		el.appendChild(dd.$('.csb-options-wrap'))
		dd.remove()
