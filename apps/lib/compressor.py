...
class UglifyJSCompressor(JsCompressor):
    ...
    def output(self, mode='file', forced=False):
        ...
        try:
            ...
            uglifyjs_binary = getattr(settings, 'UGLIFY_JS_BINARY',
                                      os.path.join(settings.BASE_DIR, 'node_modules/uglify-js/bin/uglifyjs'))
            command = [uglifyjs_binary] + [basename for filename, basename in filenames] + [
                '-o', new_filepath,
                '--source-map', 'url="%s",root="%s"' % (settings.STATIC_URL + new_map_filepath, settings.STATIC_URL)
            ]
